...
<table class="table table-hover">
    <tr>
        <th>
            <form name="formsea" class="form-inline" role="form" method="post" action="<%=path %>/ordergoodslist">
                ...
                <div class="form-group">
                    <div class="col-sm-2">
                        <input type="text" name="nums" id="nums" class="form-control" placeholder="数量"/>
                    </div>
                </div>
...
<table class="table table-hover table-striped table-bordered">
    <tr>
        <th>门店订货列表 &nbsp; &nbsp;
 <!--
 <button type="button" onclick="location.href='<%=path %>/ordergoodsadd'" class="btn  btn-primary">添加</button>
 -->
        </th>
    </tr>
</table>
        ...
        <tr>
            ...
            <td> 
                <c:if test="${c.state=='未审核'}">
                    <a href="<%=path %>/ordergoodsupdate?id=${c.id}&state=已审核&forword=ordergoodslist" class="btn btn-info">审核</a> |
                </c:if>
                <a href="ordergoodsdel?id=${c.id}&forword=ordergoodslist" onClick="return confirm('您确定要删除吗?')" class="btn btn-default">删除</a>
            </td>
        </tr>
        </tbody>
    </c:forEach>
...
<script type="text/javascript">
    function go(index) {
        window.location.href="<%=path %>/ordergoodslist?index="+index;
    }
</script>
<table class="table table-hover table-striped table-bordered">
    <tr>
