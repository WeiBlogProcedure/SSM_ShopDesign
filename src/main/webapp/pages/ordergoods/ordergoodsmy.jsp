...
<table class="table table-hover">
    <tr>
        <th>
            <form name="formsea" class="form-inline" role="form" method="post" action="<%=path %>/ordergoodsmy">
...
<table class="table table-hover table-striped table-bordered">
    <tr>
        <th>门店订货列表 &nbsp; &nbsp; 
 <!--
 <button type="button" onclick="location.href='<%=path %>/dinghuoadd'" class="btn  btn-primary">添加</button>
 -->
        </th>
    </tr>
</table>
        ...
        <tr>
            ...
            <td>
                <c:if test="${c.state=='未审核'}">
                    <a href="<%=path %>/ordergoodsedit?id=${c.id}&forword=ordergoodsmy" class="btn btn-default">修改</a> |
                    <a href="ordergoodsdel?id=${c.id}&forword=ordergoodsmy" onClick="return confirm('您确定要删除吗?')" class="btn btn-default">删除</a>
                </c:if>
            </td>
        </tr>
        </tbody>
...
<script type="text/javascript">
    function go(index) {
        window.location.href="<%=path %>/ordergoodsmy?index="+index;
    }
</script>
