...
<table class="table table-hover">
    <tr>
        <th>
            <form name="formsea" class="form-inline" role="form" method="post" action="<%=path %>/returngoodslist">
                <div class="form-group">
                    <div class="col-sm-2">
                        <input type="text" name="name" id="name" class="form-control" placeholder="产品名称"/>
                    </div>
                </div>
                ...
                <div class="form-group">
                    <div class="col-sm-2">
                        <input type="text" name="nums" id="nums" class="form-control" placeholder="退货数量"/>
                    </div>
                </div>
...
<table class="table table-hover table-striped table-bordered">
 <tr><th>退货列表 &nbsp; &nbsp;
 <!--
 <button type="button" onclick="location.href='<%=path %>/returngoodsadd'" class="btn  btn-primary">添加</button>
 -->
</th></tr></table>
<table class="table table-hover table-striped table-bordered">
    <thead><tr>
    ...
    <c:forEach items="${requestScope.page.data}" var="c" varStatus="ss">
        ...
        <tbody><tr>
            ...
            <td>
                <c:if test="${c.state=='未审核'}">
                    <a href="<%=path %>/returngoodsupdate?id=${c.id}&state=已审核&forword=returngoodslist" class="btn btn-info">审核</a> |
                </c:if>
                <a href="returngoodsdel?id=${c.id}&forword=returngoodslist" onClick="return confirm('您确定要删除吗?')" class="btn btn-default">删除</a>
            </td>
        </tr></tbody>
    </c:forEach>
</table>
...
<script type="text/javascript">
    function go(index) {
        window.location.href="<%=path %>/returngoodslist?index="+index;
    }
</script>
