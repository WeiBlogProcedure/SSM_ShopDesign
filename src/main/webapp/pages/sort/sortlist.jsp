...
<table   class="table table-hover">
 <tr> <th>
<form name="formsea" class="form-inline" role="form" method="post" action="<%=path %>/sortlist">
  
  <div class="form-group">
    <div class="col-sm-2">
	<input type="text" name="name" id="name" class="form-control" placeholder="分类名称"/>
	</div>
  </div>
...
<table class="table table-hover table-striped table-bordered">
 <tr><th>产品分类列表 &nbsp; &nbsp;
 <!--
 <button type="button" onclick="location.href='<%=path %>/sortadd'" class="btn  btn-primary">添加</button>
 -->
</th></tr></table>
<table class="table table-hover table-striped table-bordered">
 <thead>
...
<td> 
 <a href="<%=path %>/sortedit?id=${c.id}&forword=sortlist" class="btn btn-default">修改</a> |
 <a href="sortdel?id=${c.id}&forword=sortlist" onClick="return confirm('您确定要删除吗?')" class="btn btn-default">删除</a>
 </td>
</tr></tbody>
</c:forEach> </table><script type="text/javascript">
	       function go(index)
           {
               window.location.href="<%=path %>/sortlist?index="+index;
           }
</script>
<table class="table table-hover table-striped table-bordered">
						        <tr>
