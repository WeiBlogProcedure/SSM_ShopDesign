...
<table   class="table table-hover">
 <tr> <th>
<form name="formsea" class="form-inline" role="form" method="post" action="<%=path %>/storelist">
  
  <div class="form-group">
    <div class="col-sm-2">
	<input type="text" name="name" id="name" class="form-control" placeholder="门店名称"/>
	</div>
  </div>
  
  <div class="form-group">
    <div class="col-sm-2">
	<input type="text" name="address" id="address" class="form-control" placeholder="门店地址"/>
	</div>
  </div>
  ...
  <div class="form-group">
    <div class="col-sm-2">
	<input type="text" name="tname" id="tname" class="form-control" placeholder="店长姓名"/>
	</div>
  </div>
...
<table class="table table-hover table-striped table-bordered">
 <tr><th>门店列表 &nbsp; &nbsp;
 <!--
 <button type="button" onclick="location.href='<%=path %>/storeadd'" class="btn  btn-primary">添加</button>
 -->
</th></tr></table>
<table class="table table-hover table-striped table-bordered">
 <thead>
...
<td> 
 <a href="<%=path %>/storeedit?id=${c.id}&forword=storelist" class="btn btn-default">修改</a> |
 <a href="storedel?id=${c.id}&forword=storelist" onClick="return confirm('您确定要删除吗?')" class="btn btn-default">删除</a>
 </td>
</tr></tbody>
</c:forEach> </table><script type="text/javascript">
	       function go(index)
           {
               window.location.href="<%=path %>/storelist?index="+index;
           }
</script>
<table class="table table-hover table-striped table-bordered">
						        <tr>
