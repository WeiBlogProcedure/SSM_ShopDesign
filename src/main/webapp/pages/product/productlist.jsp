...
<table   class="table table-hover">
 <tr> <th>
<form name="formsea" class="form-inline" role="form" method="post" action="<%=path %>/productlist">
  ...
  <div class="form-group">
    <div class="col-sm-2">
	<input type="text" name="number" id="number" class="form-control" placeholder="编号"/>
	</div>
  </div>
...
<table class="table table-hover table-striped table-bordered">
 <tr><th>产品列表 &nbsp; &nbsp;
 <!--
 <button type="button" onclick="location.href='<%=path %>/productadd'" class="btn  btn-primary">添加</button>
 -->
</th></tr></table>
<table class="table table-hover table-striped table-bordered">
 <thead>
...
<td> 
 <a href="<%=path %>/productedit?id=${c.id}&forword=productlist" class="btn btn-default">修改</a> |
 <a href="productdel?id=${c.id}&forword=productlist" onClick="return confirm('您确定要删除吗?')" class="btn btn-default">删除</a>
 </td>
</tr></tbody>
</c:forEach> </table><script type="text/javascript">
	       function go(index)
           {
               window.location.href="<%=path %>/productlist?index="+index;
           }
</script>
<table class="table table-hover table-striped table-bordered">
						        <tr>
